...
export class WellPipe implements PipeTransform {
    transform(entries: Entry[], complete_citizen_list: CitizenInfo): { citizen: string, rations: number }[] {
        ...
        return groupBy(
            ...
            .map((rations: { citizen: Citizen, rations: number }[]) => {
                return {
                    ...
                    citizen: rations.map((citizen: { citizen: Citizen, rations: number }) => citizen.citizen).map((citizen: Citizen) => citizen.name).join(', ')
                };
            });
    }
}
