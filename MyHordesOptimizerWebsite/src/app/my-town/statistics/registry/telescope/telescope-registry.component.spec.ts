...
import { TelescopeRegistryComponent } from './telescope-registry.component';
...
function telescopeItem(): Item {
    const item: Item = new Item();
    item.label = { fr: 'Telescope' };
    item.img = 'item/item_scope.gif';
    return item;
}
...
describe('TelescopeRegistryComponent', (): void => {
    let fixture: ComponentFixture<TelescopeRegistryComponent>;

    beforeEach(async (): Promise<void> => {
        await TestBed.configureTestingModule({
            imports: [TelescopeRegistryComponent]
        }).compileComponents();
        fixture = TestBed.createComponent(TelescopeRegistryComponent);
        fixture.componentRef.setInput('completeCitizenList', citizenInfo([newCitizen('Bob', 1), newCitizen('Alice', 2)]));
        fixture.componentRef.setInput('completeItemsList', [telescopeItem()]);
        fixture.componentRef.setInput('displayPseudo', 'simple');
    });

    it('lists a citizen mentioned in an entry that references the telescope as having taken it', (): void => {
        const entries: Entry[] = [{ hour: '10:00', entry: 'Bob a utilise le Telescope du camp' }];
    ...
    it('ignores entries that do not mention the telescope, even if they mention a known citizen', (): void => {
        const entries: Entry[] = [
            { hour: '10:00', entry: 'Bob a utilise le Telescope du camp' },
        ...
        expect(notTaken.textContent).toContain('(1)');
    });

    it('shows everyone as not having taken the telescope when registry is undefined', (): void => {
        ...
        expect(fixture.debugElement.queryAll(By.css('.not-taken mho-citizen-info')).length).toBe(2);
    });
    ...
    it('updates reactively when the registry input changes', (): void => {
        ...
        fixture.componentRef.setInput('registry', [{ hour: '10:00', entry: 'Bob a utilise le Telescope du camp' }] as Entry[]);
        fixture.detectChanges();
    ...
    it('does not react to an isolated change of completeItemsList, matching the legacy setter behavior', (): void => {
        ...
        otherItem.label = { fr: 'Longue-vue' };
        otherItem.img = 'item/item_scope.gif';
