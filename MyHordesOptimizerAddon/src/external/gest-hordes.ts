...
export function getGHMap() {
    return new Promise<any>((resolve, reject) => {
        const getArrow = (arrow) => {
            const arrow_sprite = arrow.firstChild.href.baseVal.replace(/^(.*)#/, '');
            switch (arrow_sprite) {
                case 'fleche_0':
                case 'fleche_0_b':
                case 'fleche_0_n':
        ...
        getStorageItem(mho_map_key).then((mho_map) => {
            ...
            const new_map = [];
            const map = arrFrom(map_html.querySelectorAll('.ligneCarte'));
    ...
    return new Promise<any>((resolve, reject) => {
        getStorageItem(mho_map_key).then((mho_map) => {
            if (mho_map.ruin) {
                ...
                const new_map = [];
                const rows = arrFrom(map_html.querySelector('#carteRuine')?.querySelector('tbody')?.children);
                    ...
                    .forEach((row, row_index, rows_array) => {
                        ...
                        cells.forEach((cell, cell_index, cell_array) => {
                            if (cell.classList.contains('bordCarteRuine')) {
                                ...
                                const img_path = cell.querySelector('.ruineCarte')?.firstElementChild.href.baseVal.replace(/^(.*)#/, '');
                                switch (img_path) {
                                    case 'ruineCarte_16':
