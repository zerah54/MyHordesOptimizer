        ...
        if (!state.external_app_id || state.external_app_id === '') {
            ...
            fetcherWithoutBearer(`${location.origin}/jx/soul/settings`, {
                ...
                .then((response: string) => {
                    const manual: () => void = (): void => {
                        if (!document.querySelector('.soul')) {
                            reject(response);
                            return;
                        }
                        const manual_app_id_key: string = prompt(getI18N(texts.manually_add_app_id_key));
                    ...
                    const temp_body: HTMLBodyElement = document.createElement('body');

                    if (response) {
                        temp_body.innerHTML = response;
                        const id: any = temp_body.querySelector('#app_ext');
